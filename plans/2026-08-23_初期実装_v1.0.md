...
**v1 対象（同日追記）:** ライブは **QuickDiskBench のみ**。**QuickImageView は対象外**（別途開発中）。pywinauto の ImageView example は作らない。ネイティブ Win32 例外ルールの記述は残すが、このリポジトリの v1 完了条件にしない。
...
v1 のライブ参照は **QuickDiskBench 1 本**（UAC なし、ファイルダイアログなし、`AreDevToolsEnabled` 未設定）。QuickImageView は別途開発中のため **対象外**。残りアプリは契約表のみ。v1 で全部は回さない。
...
- ライブ example spec は **QuickDiskBench（Playwright）のみ**。v1 の YAML は **本リポジトリ `examples/QuickDiskBench/` のみ**。`examples/QuickImageView/` は作らない。アプリ repo の `python/qa/` には書かない。
...
**QuickImageView は v1 対象外**（別途開発中）。ネイティブ Win32 の知見（`ui_test.py` の `menu_select`、`#32770`、`--ui-test-hidden` 禁止）は将来用に残すだけ。このリポジトリでは example spec もライブ検査もしない。
...
```text
QuickAppsTest/examples/QuickDiskBench/spec.yaml    # 正本（v1 唯一のライブ spec）
```

`examples/QuickImageView/` は **作らない**（対象外）。アプリ repo の `python/qa/` にも v1 では書かない。
...
### QuickImageView spec — v1 対象外（別途開発中）

この節は将来用のメモ。v1 では YAML も example ディレクトリも作らない。
...
3. 第一ライブ: DiskBench structural / i18n / help / heartbeat。
4. ImageView は対象外。再開するまで第二ライブにしない。
...
5. **QuickImageView は v1 対象外。** 別途開発中。ネイティブ example は後回し。OCR/pixel は共有コアに入れない。
...
12. **v1 ライブ spec は DiskBench 1 本。ImageView は対象外。6 アプリ全対応と MarkPDF qa 全移行は後続。**
...
| 2 | spec.yaml をアプリ repo にいつ入れるか | **v1 は `examples/QuickDiskBench/spec.yaml` のみ。** アプリ repo の `python/qa/` には書かない。 |
| 3 | 7Zip/FileCopy の CDP スパイク | **post-v1 の調査**。v1 ゲートではない。 |
| 4 | ImageView をいつ入れるか | **入れない（別途開発中）。** example / PR3 の ImageView 成功条件は外す。 |

「HTML をどのバックエンドでクリックするか」「wait_message に cursor が要るか」も実装選択として閉じ済み。
...
### PR3 — pywinauto backend（DiskBench 窓タイトルのみ。ImageView なし）

- **Title:** `Add pywinauto backend for DiskBench window title`
- **Files:** `backends/pywinauto_backend.py`, `session.py`（pywinauto 差し込み）, DiskBench spec に `native.window_title` を追加
- **Dependencies:** PR2
- **Description:** ImageView example は **作らない**。成功: DiskBench の Win32 タイトルが HTML `<title>` より長い `Native Storage Benchmark` 側であること。SendMessage 禁止。OCR なし。

### PR4 — Appium backend + skip-if-unavailable + HWND attach

- **Title:** `Add optional Appium backend via appTopLevelWindow`
- **Files:** `backends/appium_backend.py`, DiskBench spec の `via: appium` 行, `tests/test_appium_skip.py`
- **Dependencies:** PR2（HWND は pywinauto が無くても FindWindow(window_class) で取れるが、PR3 があると楽）
- **Description:** `WindowsOptions.app_top_level_window`、`Remote("http://127.0.0.1:4723")`（`/wd/hub` なし）。TCP 1s fail-fast、createSession 10s。contains 照合。CSS locator 拒否。成功: (a) Appium 無しで Playwright が緑（SKIP が正） (b) **Appium 2 が 4723 で生きているときだけ** DiskBench の Name contains が pass。v1 緑に Appium pass も ImageView も必須にしない。
...
- **Dependencies:** **PR2**（PR3/PR4 は任意。未知 via は SKIP）
- **Description:** `via` 推論、cursor、`--spec --exe --extra-arg --skip-appium --trace --report-dir`、exit code。成功: DiskBench example を CLI 一発。ImageView は対象外。
